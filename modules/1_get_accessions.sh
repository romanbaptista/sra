...
esearch -db bioproject -query "${BIOPROJECT}" \
  ...
  | efetch -format uid \
  > "${OUTPUT_DIR}/biosample_uids.txt"

# Get number of UIDs
COUNT_UID=$(wc -l < "${OUTPUT_DIR}/biosample_uids.txt")

echo "  ${COUNT_UID} BioSample UIDs extracted"
...
efetch -db biosample -format docsum \
    < "${OUTPUT_DIR}/biosample_uids.txt" \
    > "${OUTPUT_DIR}/biosample_docsum.xml"
...
cat "${OUTPUT_DIR}/biosample_docsum.xml" \
  | xtract -pattern DocumentSummary -element Accession \
  > "${OUTPUT_DIR}/biosample_samn_accessions.txt"

# Get number of SAMN IDs
COUNT_SAMN=$(wc -l < "${OUTPUT_DIR}/biosample_samn_accessions.txt")
...
COUNT_SRR=$(wc -l < "${OUTPUT_DIR}/biosample_srr_accessions.txt")
